...
export default class PouchDBUtil {
  ...
  public static searchWithOr = async (table: string, queries: any[] = [], orderBy: string = '', orderType: 'asc' | 'desc' = 'asc', lastElement: any, pageSize: number = 50) => {
    ...
    if (queries.length > 0) {
      for (let query of queries) {
        let result = await PouchDBUtil.search(table, [query], orderBy, orderType, lastElement, pageSize);
        resultMerged = mergeArrays(result, resultMerged, 'id');
      }
    } else {
      return await PouchDBUtil.search(table, [], orderBy, orderType, lastElement, pageSize);
    }
